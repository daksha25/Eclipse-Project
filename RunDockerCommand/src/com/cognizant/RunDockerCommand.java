...
public class RunDockerCommand {
	...
	public static void main(String args[]) throws IOException {
		...
		String strdata = null;
		String command1 = "docker history jtomcat7:latest";
		...
		while ((strLine = br.readLine()) != null) {
			...
			if (strLine.contains("/bin/sh -c")) {
				...
				sizeoflayer.add(part1.get(lastindex - 2)+part1.get(lastindex - 1)); //size of layers
								part1.clear();
							}
			
		}
		System.out.println("Size of layer"+sizeoflayer);
		...
		for (int i = 0; i < command2.size(); i++) {
			System.out.println(command2.get(i));
			if(command2.get(i).contains("(nop)"))
			{
				//System.out.println("inside for");
				finalcommand.add(command2.get(i).replace("#(nop) ", " "));
				
			}
			else
				finalcommand.add(command2.get(i));
		}
